...
namespace DevSAAS.Core.Notification.Templates.Verification;
...
public class VerificationCode
{
    ...
    public void Send()
    {
        if (_smsService != null)
        {
            var message = LanguageService.Get("VerificationSms").Replace(":otp", _code).Replace(":title", LanguageService.Get("Title"));
        ...
        email = email.Replace("[type_of_action]", LanguageService.Get("VerificationCode"))
            ...
            .Replace("[confirm_account_hint]", LanguageService.Get("ConfirmAccountHint").Replace(":title", LanguageService.Get("Title")))
            .Replace("[mail_disclaimer]", LanguageService.Get("MailDisclaimer").Replace(":action", LanguageService.Get("VerificationCode")))
            .Replace("[code]", _code)
